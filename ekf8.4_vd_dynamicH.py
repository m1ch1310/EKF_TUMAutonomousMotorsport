...
class EKF_Frenet:
    def __init__(self, x_init, params, sensor_init, ego_init):
        ...
        self.kf.F[0, 3] = self.dt_s  # s' = s + v_s * dt
        self.kf.F[1, 4] = self.dt_s  # d' = d + v_d * dt
        self.kf.F[3, 5] = self.dt_s  # v_s' = v_s + a_s * dt
        self.kf.F[4, 6] = self.dt_s  # v_d' = v_d + a_d * dt
    ...
    def update_measurementnoise(self, ego_pose, x, sensor):
        ...
        self.kf.H = np.eye(self._n_dof)[:len(self._measure_covs_dict[sensor]), :]
...
if __name__ == "__main__":
    ...
    params = {
    "timestep_s": dt,
    "P_init": np.eye(7) * 0.7,  # Weitere Reduzierung der Anfangskovarianz für noch stabilere Schätzungen
    "measure_covs_dict": {"lidar": np.diag([0.015, 0.08])},  # Noch geringere Messunsicherheiten für s
    "H_indices": {"lidar": ([0, 1], [0, 1])},
    "process_noise": [0.05, 0.045, 0.0018, 0.0018, 0.0007, 0.00008, 0.00005]  # Weiter optimiertes Prozessrauschen für s
}
...
df_lidar.to_excel(r'C:\Users\2d\Engineering\Studium\Master\3Semester\Semesterarbeit\ekf8.4_input_sd_vdad_ohneas_dynamicH.xlsx', index=False)
