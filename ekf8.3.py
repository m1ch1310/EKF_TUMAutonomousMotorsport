...
params = {
    "timestep_s": dt,
    "P_init": np.eye(6) * 0.8,  # Weitere Reduzierung der Anfangskovarianz für stabilere Schätzungen
    "measure_covs_dict": {"lidar": np.diag([0.02, 0.1])},  # Noch geringere Messunsicherheiten für s
    "H_indices": {"lidar": ([0, 1], [0, 1])},
    "process_noise": [0.06, 0.05, 0.002, 0.002, 0.0008, 0.0001]  # Weiter optimiertes Prozessrauschen für s
}
...
df_lidar.to_excel(r'C:\Users\2d\Engineering\Studium\Master\3Semester\Semesterarbeit\ekf8.3_input_sd.xlsx', index=False)
